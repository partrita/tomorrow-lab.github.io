...
## 2025-05-21 - [Handling Insecure Deserialization in Educational Notebooks]
**Vulnerability:** The tutorial notebook `da_scanpy_workshop_07.ipynb` uses `pickle.load()` to deserialize an externally downloaded `.pkl` file. This is an insecure deserialization vulnerability (CWE-502), as unpickling maliciously crafted data can result in arbitrary code execution.
**Learning:** In workshop or educational contexts where notebooks download data from external upstream sources, changing the data format to a secure one (like JSON, CSV, or Parquet) may not be possible without breaking the tutorial. In these specific cases, completely removing the insecure code might prevent users from following along, and updating the upstream source may be out of our control.
**Prevention:** When an insecure function like `pickle.load()` must be used for educational purposes due to upstream constraints, always insert a prominent `🚨 SECURITY WARNING` directly in the code cell. The warning should explain the risk of arbitrary code execution, explicitly state that it should not be used in production with untrusted sources, and suggest secure alternatives.
